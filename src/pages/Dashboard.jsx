...
export default function Dashboard() {
  ...
  const loadData = async () => {
    ...
    const { data: rep } = await supabase.from('daily_reports').select('*, profiles(full_name)').order('created_at', { ascending: false }).limit(5)
    setProjects(proj || [])
    setReports(rep || [])
    setLoading(false)
  }
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={k.label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px 20px', textAlign: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            ...
            projects.slice(0, 5).map(p => {
              ...
              return (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <span style={{ padding: '2px 10px', borderRadius: 99, fontSize: 11, fontWeight: 500, background: s.bg, color: s.text }}>{s.label}</span>
                </div>
              )
            })
          )}
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20 }}>
          ...
          {reports.length === 0 ? (
            <div style={{ color: 'var(--text2)', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>No hay reportes aún</div>
          ) : (
            reports.map(r => (
              <div key={r.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{r.profiles?.full_name || 'Sin nombre'}</div>
                  {r.has_blocker && (
                    <span style={{ padding: '2px 8px', borderRadius: 99, fontSize: 10, background: 'var(--red-bg)', color: 'var(--red-text)' }}>Bloqueado</span>
                  )}
                </div>
                <div style={{ fontSize: 12, color: 'var(--text2)' }}>{r.report_date} · {r.tasks_done}/{r.tasks_total} tareas</div>
                {r.ai_summary && (
                  <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 4 }}>{r.ai_summary.substring(0, 80)}...</div>
                )}
              </div>
            ))
          )}
