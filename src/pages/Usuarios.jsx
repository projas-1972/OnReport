    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={() => { setShowModal(true); setError('') }}
          style={{ padding: '8px 16px', background: '#f0f0f0', color: '#0f0f0f', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
          + Agregar usuario
        </button>
      </div>

      <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1.5fr 120px', gap: 12, padding: '12px 20px', background: '#242424', fontSize: 11, color: '#888' }}>
        ...
        {users.length === 0 ? (
          ...
          <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1.5fr 120px', gap: 12, padding: '14px 20px', borderTop: '1px solid #2a2a2a', alignItems: 'center' }}>
            ...
            <select value={u.role || 'capataz'} onChange={e => updateRole(u.id, e.target.value)}
              style={{ padding: '4px 8px', background: roleBg[u.role] || '#1a1a1a', border: '1px solid #333', borderRadius: 6, color: roleColors[u.role] || '#888', fontSize: 12 }}>
          ...
          <div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, padding: 24, width: 440, maxWidth: '95vw' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div><label style={labelStyle}>Nombre completo *</label>
                <input style={inputStyle} value={form.full_name} onChange={e => setForm({...form, full_name: e.target.value})} placeholder="Juan Méndez" />
              </div>
              ...
              <div><label style={labelStyle}>Teléfono</label>
                <input style={inputStyle} value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="+56 9 1234 5678" />
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
                ...
                <button onClick={handleSave} disabled={saving}
                  style={{ padding: '8px 20px', background: saving ? '#333' : '#f0f0f0', color: saving ? '#888' : '#0f0f0f', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: saving ? 'default' : 'pointer' }}>
